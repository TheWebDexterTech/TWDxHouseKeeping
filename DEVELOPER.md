# TWDxHouseKeeping — Developer Reference
...
| HK-21 | Pages projects list: no pagination params (endpoint rejects `page`/`per_page`) | `cleanup.js:~390` |
| HK-22 | Workers cleanup at script level via `DELETE /scripts/{id}` (version DELETE unsupported) | `cleanup.js:~460` |
| HK-23 | Workers active-deployment guard: check `/deployments` before delete; skip if any version >0% traffic | `cleanup.js:~475` |
| HK-24 | Pages aliased-deployment guard: filter `aliases?.length > 0` before eligible slice (CF error 8000035) | `cleanup.js:~421` |
| HK-25 | Workers per-worker DELETE isolation: catch per-worker errors, warn and continue (CF 10064 Queue consumer) | `cleanup.js:~493` |
| HK-26 | Pages per-deployment DELETE isolation: catch per-deployment errors, warn and continue (CF 8000035 aliased) | `cleanup.js:~431` |
| HK-27 | Workers HK-23 safe default: empty/null `versions` array treated as active (old Upload-API workers) | `cleanup.js:~481` |
| HK-28 | User repos: use `/user/repos?visibility=all&affiliation=owner` (authenticated) instead of `/users/{u}/repos` (public-only) | `cleanup.js:~557` |
| HK-29 | Git history auto-discovery: `git_history_repos` is optional; falls back to repos discovered from `users`/`orgs` when omitted | `cleanup.js:~716` |
...
```json
{
  ...
  "github": [{
    ...
    "git_history_repos": "string[] — optional; auto-discovers from users/orgs if omitted; format 'owner/repo'",
