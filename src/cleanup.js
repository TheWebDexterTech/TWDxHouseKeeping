...
/**
 * Sanitize an error message before logging.
 *
 * CodeQL rule js/clear-text-logging flags console calls that include data
 * derived from process.env (tokens, account IDs, secrets). Even though we
 * never log tokens directly, failed API responses can echo partial credentials.
 *
 * This function truncates and redacts before anything reaches the log.
 */
    ...
    if (!Array.isArray(config.github)) {
      ...
      config.github.forEach((acc, i) => {
        ...
        if (acc.clean_git_history) {
          if (!Array.isArray(acc.git_history_repos) || !acc.git_history_repos.length) {
            errors.push(`github[${i}]: 'clean_git_history' requires 'git_history_repos' to be a non-empty array`);
          } else if (!acc.git_history_repos.every((r) => {
            if (typeof r !== "string") {return false;}
            const idx = r.indexOf("/");
            // Must have exactly one slash with non-empty owner and repo on both sides
      ...
      const allProjects = [];
      {
        ...
        while (true) {
          const res = await apiFetch(
            `https://api.cloudflare.com/client/v4/accounts/${accountId}/pages/projects?per_page=25&page=${page}`,
            { headers }
          );
    ...
    try {
      ...
      for (const worker of workers?.result ?? []) {
        ...
        if (Array.isArray(raw)) {
          ...
          console.warn(`   ⚠️  Unexpected versions response shape for worker "${worker.id}": ${JSON.stringify(raw).slice(0, 120).replace(/[A-Za-z0-9+/]{32,}/g, "[REDACTED]")}`);
          skipped++;
          continue;
        }
...
async function cleanGitHub(acc, report) {
  ...
  // ── Git History ────────────────────────────────────────────────────────────
  if (acc.clean_git_history) {
    const historyRepos    = acc.git_history_repos ?? [];
      ...
      try {
        ...
        if (keepHistoryCount === 0) {
          ...
          if (!DRY_RUN) {
            ...
            // Re-create each kept commit preserving its tree and message
            ...
            // Force-update branch ref to the last re-created commit
            await apiFetch(
              `https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${branch}`,
              {
                method: "PATCH",
