...
describe("validateConfig — invalid", () => {
  ...
  throws(
    JSON.stringify({ github: [{ token: "t", label: "l", users: ["u"], clean_git_history: true }] }),
    "throws when clean_git_history=true but git_history_repos missing"
  );
  throws(
    JSON.stringify({
      github: [{ token: "t", label: "l", users: ["u"], clean_git_history: true, git_history_repos: [] }],
    }),
    "throws when git_history_repos is empty array"
  );
  throws(
    JSON.stringify({
