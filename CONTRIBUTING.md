...
**Prerequisites:** Node.js 20+. No `npm install` needed.
...
7. If function locations in `src/cleanup.js` change significantly, update the Section Map and Function Index in `CLAUDE.md`.
...
Every behaviour change or safety fix gets a HK-ID. The current highest is **HK-20**.

When adding a new one:
1. Assign `HK-21` (or the next available number).
2. Add it to the header comment block in `src/cleanup.js`.
3. Add it to the HK Feature Map table in `CLAUDE.md`.
